...
class ProductDetailViewModel {
    ...
    func loadImage() {
        guard let imageURL = product.imageURL, let urlInstance = URL(string:imageURL.addingPercentEncoding(withAllowedCharacters:NSCharacterSet.urlQueryAllowed)!), let view = productDetailView else { return }
        
        let maxDimentionInPixels = getMaxDimentionInPixelsFrom(view.imageView, scale: 2.0)
        
        imageProcessor.downsampleAsync(product.uuid, imageURL: urlInstance, maxDimentionInPixels: maxDimentionInPixels) { image in
